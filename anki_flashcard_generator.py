#!/usr/bin/env python3
"""
Generic Anki flashcard generator with optional Notion integration.

Required environment variables:
  - OPENAI_API_KEY

Optional environment variables:
  - FLASHCARD_SOURCE (default: stdin)               # stdin | text | file | notion
  - FLASHCARD_TITLE (default depends on source)
  - FLASHCARD_TEXT (used when source=text)
  - FLASHCARD_FILE (used when source=file)
  - NOTION_TOKEN (required when source=notion)
  - NOTION_PAGE_ID (required when source=notion, or pass --page-id)
  - OPENAI_MODEL (default: gpt-4.1-mini)
  - OPENAI_TEMPERATURE (default: 0.2)
  - CARDS_PER_CHUNK (default: 4)
  - MAX_CHARS_PER_CHUNK (default: 3500)
  - ANKI_CONNECT_URL (default: http://127.0.0.1:8765)
  - ANKI_DECK (default: LLM Flashcards)
...
def slugify(value: str) -> str:
    ...
    return normalized or "flashcards"
...
def markdown_text_to_sections(text: str, default_title: str) -> list[Section]:
    stripped = text.strip()
    if not stripped:
        return []
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate Anki flashcards from multiple sources.")
    parser.add_argument(
        "--source",
        choices=["stdin", "text", "file", "notion"],
        default=get_env("FLASHCARD_SOURCE", "stdin"),
        help="Input source for content.",
    )
    parser.add_argument("--title", default=get_env("FLASHCARD_TITLE"), help="Optional source title override.")
    parser.add_argument("--text", default=get_env("FLASHCARD_TEXT"), help="Raw text input when --source text.")
    parser.add_argument("--file", default=get_env("FLASHCARD_FILE"), help="Path to input file when --source file.")
    parser.add_argument("--page-id", default=get_env("NOTION_PAGE_ID"), help="Notion page ID when --source notion.")
    parser.add_argument("--deck", default=get_env("ANKI_DECK", "LLM Flashcards"))
    ...
    parser.add_argument("--cards-per-chunk", type=int, default=parse_int(get_env("CARDS_PER_CHUNK"), 4))
    parser.add_argument(
        "--max-chars-per-chunk",
        type=int,
        default=parse_int(get_env("MAX_CHARS_PER_CHUNK"), 3500),
    )
    parser.add_argument("--temperature", type=float, default=parse_float(get_env("OPENAI_TEMPERATURE"), 0.2))
    parser.add_argument("--dry-run", action="store_true", help="Generate cards but do not upload to Anki.")
    return parser


def read_sections_from_source(args: argparse.Namespace, notion_token: str | None) -> tuple[str, list[Section]]:
    if args.source == "notion":
        if not notion_token:
            raise RuntimeError("NOTION_TOKEN is required when --source notion.")
        if not args.page_id:
            raise RuntimeError("NOTION_PAGE_ID (or --page-id) is required when --source notion.")

        notion = NotionClient(notion_token)
        page = notion.retrieve_page(args.page_id)
        source_title = args.title or extract_page_title(page)
        blocks = notion.fetch_blocks_recursive(args.page_id)
        if not blocks:
            raise RuntimeError("No block content found in the selected Notion page.")
        sections = blocks_to_sections(blocks)
        return source_title, sections

    if args.source == "file":
        if not args.file:
            raise RuntimeError("--file is required when --source file.")
        path = Path(args.file)
        if not path.exists():
            raise RuntimeError(f"Input file not found: {path}")
        text = path.read_text(encoding="utf-8")
        source_title = args.title or path.stem.replace("_", " ").strip() or "File Input"
        sections = markdown_text_to_sections(text, default_title=source_title)
        return source_title, sections

    if args.source == "text":
        text = args.text or ""
        if not text.strip():
            raise RuntimeError("--text (or FLASHCARD_TEXT) is required when --source text.")
        source_title = args.title or "Text Input"
        sections = markdown_text_to_sections(text, default_title=source_title)
        return source_title, sections
    ...
    if args.source == "stdin":
        if sys.stdin.isatty():
            raise RuntimeError(
                "Source 'stdin' selected, but no piped input found. Use: cat notes.md | python anki_flashcard_generator.py"
            )
        text = sys.stdin.read()
        if not text.strip():
            raise RuntimeError("No input was received from stdin.")
        source_title = args.title or "Stdin Input"
        sections = markdown_text_to_sections(text, default_title=source_title)
        return source_title, sections
...
def main() -> int:
    ...
    if not openai_key:
        print("Missing required configuration: OPENAI_API_KEY", file=sys.stderr)
        return 1

    try:
        source_title, sections = read_sections_from_source(args, notion_token)
        sections = split_sections(sections, max_chars=args.max_chars_per_chunk)
        if not sections:
            print("No usable section content found in the selected source.", file=sys.stderr)
            return 1
        ...
        base_tags = [slugify(source_title), "llm_generated"]
        ...
        print(f"Generated {len(all_cards)} unique cards from '{source_title}'.")
        if args.dry_run:
